...
import {
  ...
  CalendarDays, Pin, X, Sun, Layers, Wrench,
} from "lucide-react";
import {
  getAllMECHCoursesForDisplay, getAllMECHCourses,
  MECH_PREREQUISITES, MECH_ELECTIVE_BUNDLES, MECH_COMP_STUDIES_CATEGORIES, OPTION_LABELS,
  type PlanCourse, type MECHOption,
} from "../_data/mech-courses";
import {
...
function CourseCheckRow({ course, checked, onChange }: {
  ...
  return (
    <label className={cn(
      ...
      {course.summerEligible && <span title="Summer eligible"><Sun className="h-3 w-3 shrink-0 text-amber-500" /></span>}
...
function TermSection({ slot, pinnedIds, onTogglePin, onCapacityChange, onAddPin, onEject, availableToPin }: {
  slot: TermSlot; pinnedIds: ReadonlySet<string>;
  onTogglePin: (id: string) => void; onCapacityChange: (label: string, cap: number) => void;
  onAddPin: (id: string, year: number, term: 1 | 2) => void; onEject: (id: string, term: 1 | 2) => void;
    ...
    <div className={cn("px-5 py-4", isSummer && "bg-amber-50/40 dark:bg-amber-950/10")}>
      <div className="mb-3 flex items-center justify-between gap-3">
        <div className="flex items-center gap-2">
          ...
          <div>
            <p className={cn("text-xs font-bold uppercase tracking-widest", isSummer ? "text-amber-700 dark:text-amber-400" : "text-muted-foreground")}>
      ...
      <div className="flex flex-wrap gap-1.5">
        {slot.courses.map((c) => (
          <CoursePill key={c.id} course={c} pinned={pinnedIds.has(c.id)}
            onPin={() => onTogglePin(c.id)}
            onEject={isSummer ? undefined : () => onEject(c.id, slot.term)} />
        ))}
        {slot.courses.length === 0 && <p className="text-xs italic text-muted-foreground">No courses scheduled this term.</p>}
      </div>
      {!isSummer && (
        <div className="mt-3">
          {showPinPicker ? (
            <div className="flex items-center gap-2">
              <select className="flex-1 rounded-lg border border-border bg-background px-2 py-1.5 text-xs text-foreground focus:outline-none focus:ring-1 focus:ring-foreground dark:bg-card"
                defaultValue="" onChange={(e) => { if (e.target.value) { onAddPin(e.target.value, slot.academicYear, slot.term); setShowPinPicker(false); } }}>
                <option value="" disabled>Select a course to pin here…</option>
                {availableToPin.map((c) => <option key={c.id} value={c.id}>{c.code} — {c.name}</option>)}
              </select>
...
function YearPlanBox({ year, currentVariant, variantIdx, totalVariants, pinnedIds, onTogglePin, onCapacityChange, onAddPin, onEject, availableToPin, defaultOpen }: {
  year: number; currentVariant: YearVariant; variantIdx: number; totalVariants: number;
  pinnedIds: ReadonlySet<string>; onTogglePin: (id: string) => void;
    ...
    <div className="w-full overflow-hidden rounded-2xl border border-border bg-card shadow-sm">
      <button type="button" onClick={() => setOpen((o) => !o)}
        ...
        <div className="flex-1 min-w-0">
          ...
          <p className="text-xs text-muted-foreground">{currentVariant.label} · {totalCourses} courses · {totalCr} credits</p>
        </div>
        <div className="hidden sm:flex items-center gap-2 shrink-0">
          <div className="h-1.5 w-24 overflow-hidden rounded-full bg-secondary">
            <div className="h-full rounded-full bg-foreground transition-all" style={{ width: `${Math.min(100, (totalCr / 42) * 100)}%` }} />
          </div>
...
function TakenYearBox({ year, courses, completedIds, onToggle }: {
  year: number; courses: PlanCourse[]; completedIds: ReadonlySet<string>; onToggle: (id: string, v: boolean) => void;
}) {
  void year;
    ...
    <div className="border-t border-border px-5 pb-5 pt-4 space-y-1.5">
      <div className="mb-2 flex gap-3">
        <button type="button" onClick={() => courses.forEach((c) => onToggle(c.id, true))}
          className="text-xs text-muted-foreground underline underline-offset-2 hover:text-foreground">Mark all done</button>
        <button type="button" onClick={() => courses.forEach((c) => onToggle(c.id, false))}
          className="text-xs text-muted-foreground underline underline-offset-2 hover:text-foreground">Unmark all</button>
        <span className="ml-auto text-xs text-muted-foreground">{done} / {courses.length} done</span>
      </div>
      {courses.map((c) => <CourseCheckRow key={c.id} course={c} checked={completedIds.has(c.id)} onChange={onToggle} />)}
    </div>
  );
}
    ...
    <div className="rounded-2xl border border-border bg-card shadow-sm overflow-hidden">
      <button type="button" onClick={() => setOpen((o) => !o)}
        ...
        <div className="flex-1 min-w-0">
          ...
          <p className="text-xs text-muted-foreground">Min 20 credits · Click to expand</p>
        </div>
        {open ? <ChevronUp className="h-5 w-5 shrink-0 text-muted-foreground" /> : <ChevronDown className="h-5 w-5 shrink-0 text-muted-foreground" />}
      </button>
      {open && (
        <div className="border-t border-border px-5 pb-5 pt-4 space-y-4">
          <p className="text-xs text-muted-foreground">
            WRDS 150, MECH 226 (Communications), and MECH 400 (Professional Practice) together satisfy most
            Complementary Studies requirements. The Y3 CS elective slots fill the remaining credits.
          </p>
          <div className="flex flex-wrap gap-1.5">
            {["WRDS 150 ✓", "MECH 226 ✓", "MECH 400 ✓", "Y3 CS Elective x2 ✓"].map((t) => (
              <span key={t} className="rounded-full border border-emerald-300 bg-emerald-50 px-2.5 py-0.5 text-[11px] font-semibold text-emerald-800 dark:border-emerald-700 dark:bg-emerald-900/30 dark:text-emerald-200">{t}</span>
            ))}
          </div>
          <div className="space-y-1.5">
            {MECH_COMP_STUDIES_CATEGORIES.map((cat, i) => (
              <div key={i} className="rounded-xl border border-border overflow-hidden">
                <button type="button" onClick={() => setOpenCat(openCat === i ? null : i)}
                  ...
                  <div className="flex items-center gap-2">
                    <span className="flex h-5 w-5 shrink-0 items-center justify-center rounded-full bg-emerald-100 text-[10px] font-bold text-emerald-700 dark:bg-emerald-900/40 dark:text-emerald-400">{i + 1}</span>
                    <span className="text-xs font-semibold text-foreground">{cat.label}</span>
                  </div>
                  <div className="flex items-center gap-2 shrink-0">
                    <span className="rounded-full bg-secondary px-2 py-0.5 text-[10px] font-bold text-muted-foreground">min {cat.minCredits} cr</span>
                    {openCat === i ? <ChevronUp className="h-3.5 w-3.5 text-muted-foreground" /> : <ChevronDown className="h-3.5 w-3.5 text-muted-foreground" />}
                  </div>
                </button>
                {openCat === i && (
                  <div className="border-t border-border px-4 pb-3 pt-2.5 bg-secondary/20">
                    <div className="flex flex-wrap gap-1">
                      {cat.examples.map((ex) => <span key={ex} className="rounded-md border border-border bg-card px-2 py-0.5 text-[11px] text-foreground">{ex}</span>)}
                    </div>
                  </div>
                )}
              </div>
...
export default function MECHPlanner() {
  const planRef = useRef<HTMLDivElement>(null);

  const [option, setOption] = useState<MECHOption>("general");
  ...
  const displayCourses = useMemo(
    () => getAllMECHCoursesForDisplay(option, activeBundleIdx),
    [option, activeBundleIdx]
  );

  const allCourses = useMemo(
    () => getAllMECHCourses(option, activeBundleIdx, completedSet),
    [option, activeBundleIdx, completedSet]
  );

  /**
   * Stable reference for the auto-fill useEffect. Uses bundle 0 and only
   * changes when the option changes — never when the tech elective bundle
   * index changes. Prevents Year 4 arrow clicks from resetting the plan.
   */
  const stableCoursesForStanding = useMemo(
    () => getAllMECHCoursesForDisplay(option, 0),
    [option]
  );
  ...
  // Reset bundle when option changes
  useEffect(() => { setActiveBundleIdx(0); }, [option]);
  ...
  const safeBundle = Math.min(activeBundleIdx, bundles.length - 1);
  const activeBundle = bundles[safeBundle];

  const rebuildForBundle = useCallback((bundleIdx: number, capOverride?: Record<string, number>) => {
    return buildDegreePlan({
      allCourses: getAllMECHCourses(option, bundleIdx, completedSet),
      completedIds: completedSet, startYear: standing, startTerm: 1,
      defaultCoursesPerTerm: defaultLoad,
      termCapacities: capOverride ?? termCapacities,
      totalDegreeYears: totalYears, pinnedCourses, termExclusions,
    });
  }, [option, completedSet, standing, defaultLoad, termCapacities, totalYears, pinnedCourses, termExclusions]);

  const handleBundleChange = useCallback((newIdx: number) => {
    const clamped = Math.max(0, Math.min(bundles.length - 1, newIdx));
    setActiveBundleIdx(clamped);
    if (generated) setPlan(rebuildForBundle(clamped));
  }, [bundles.length, generated, rebuildForBundle]);

  const rebuildPlan = useCallback((overrides: { pins?: CoursePin[]; caps?: Record<string, number>; excls?: TermExclusion[] }) => {
    return buildDegreePlan({
      allCourses, completedIds: completedSet, startYear: standing, startTerm: 1,
      defaultCoursesPerTerm: defaultLoad, termCapacities: overrides.caps ?? termCapacities,
      totalDegreeYears: totalYears, pinnedCourses: overrides.pins ?? pinnedCourses,
  ...
  const handleCapacityChange = useCallback((label: string, cap: number) => {
    setTermCapacities((prev) => { const next = { ...prev, [label]: cap }; setPlan(rebuildPlan({ caps: next })); return next; });
  }, [rebuildPlan]);

  const handleTogglePin = useCallback((courseId: string) => {
    setPinnedCourses((prev) => { const next = prev.filter((p) => p.courseId !== courseId); setPlan(rebuildPlan({ pins: next })); return next; });
  }, [rebuildPlan]);
  ...
  const toggleCompleted = useCallback((id: string, checked: boolean) => {
    setCompletedIds((prev) => checked ? (prev.includes(id) ? prev : [...prev, id]) : prev.filter((x) => x !== id));
    setGenerated(false);
  }, []);

  const handleGenerate = () => {
    const errors = validatePrerequisites(completedSet, displayCourses, MECH_PREREQUISITES);
    setPrereqErrors(errors);
    if (errors.length > 0) { setGenerated(false); setPlan([]); return; }
    const result = buildDegreePlan({
      allCourses, completedIds: completedSet, startYear: standing, startTerm: 1,
      defaultCoursesPerTerm: defaultLoad, termCapacities, totalDegreeYears: totalYears,
  ...
  return (
    <div className="mx-auto max-w-3xl space-y-7 px-4 py-10 sm:px-6">
      ...
      <div>
        <p className="mb-1 text-xs font-semibold uppercase tracking-widest text-muted-foreground">B.A.Sc. · Applied Science · MECH</p>
        <h1 className="text-3xl font-extrabold tracking-tight text-foreground sm:text-4xl">Degree Planner</h1>
        <p className="mt-1 text-sm text-muted-foreground">
          Mechanical Engineering · {OPTION_LABELS[option]} · ~156 credit program
        </p>
      </div>
      ...
      <div className="rounded-2xl border border-border bg-card p-6 shadow-sm space-y-5">
        ...
        <div className="grid grid-cols-2 gap-4 sm:grid-cols-4">
          <Field label="Specialization Option">
            <Select value={option} onChange={(v) => { setOption(v as MECHOption); setGenerated(false); }}>
              <option value="general">General (No Option)</option>
              <option value="aerospace">Aerospace</option>
              <option value="bio">Biomechanics & Med Devices</option>
              <option value="energy">Energy & Environment</option>
              <option value="mechatronics">Mechatronics</option>
              <option value="naval">Naval Architecture & Marine</option>
            </Select>
          </Field>
        ...
        <div className="flex flex-wrap gap-x-4 gap-y-1 border-t border-border pt-3">
          <p className="text-xs text-muted-foreground flex items-center gap-1.5"><GraduationCap className="h-3.5 w-3.5" />{standingLabel}-year standing</p>
          <p className="text-xs text-muted-foreground flex items-center gap-1.5"><CalendarDays className="h-3.5 w-3.5" />{Math.max(1, totalYears - standing + 1)} years remaining</p>
          <p className="text-xs text-muted-foreground">{remainingCount} courses to schedule</p>
          <p className={cn("text-xs font-semibold flex items-center gap-1.5", option === "general" ? "text-muted-foreground" : "text-amber-600 dark:text-amber-400")}>
            <Wrench className="h-3.5 w-3.5" />{OPTION_LABELS[option]}
          </p>
        </div>
      </div>

      {/* MECH 2 early-start note */}
      <div className="flex items-start gap-3 rounded-xl border border-amber-300 bg-amber-50 px-4 py-3 dark:border-amber-700 dark:bg-amber-950/30">
        ...
        <p className="text-xs text-amber-800 dark:text-amber-200">
          <strong>MECH 2 starts early.</strong> Year 2 begins the week <em>before</em> standard Term 1 — attendance is mandatory from day one.
          If you live in UBC Housing, apply for early arrival. MECH 220 is completed during this early-start week.
          All Year 1 prereqs (APSC 160, MATH 100/101/152, PHYS 157/158/170, WRDS 150) must be done by <strong>August 1st</strong>.
        </p>
      </div>
      ...
      {option !== "general" && (
        <div className="flex items-start gap-3 rounded-xl border border-amber-300 bg-amber-50 px-4 py-3 dark:border-amber-700 dark:bg-amber-950/30">
          ...
          <p className="text-xs text-amber-800 dark:text-amber-200">
            <strong>{OPTION_LABELS[option]} option</strong> — admission to this option occurs at the end of Year 2 based on academic performance and interest.
            Option-specific courses are shown in <span className="font-semibold">amber</span> pills; tech elective bundles navigate with ← → on Year 4.
          </p>
        </div>
      )}
          ...
          <div className="border-t border-border px-5 pb-5 pt-4">
            ...
            <div className="grid grid-cols-4 gap-3 mb-4">
              {displayByYear.map(([year, courses]) => {
                ...
                return (
                  <button key={year} type="button" onClick={() => setOpenTakenYear(isOpen ? null : year)}
                    ...
                    {done === courses.length && courses.length > 0 && <p className="text-[10px] font-bold text-emerald-600 dark:text-emerald-400">✓ All done</p>}
                  </button>
                );
              })}
            </div>
              ...
              return (
                <div className="rounded-2xl border border-foreground overflow-hidden">
                  <div className="flex items-center justify-between px-5 py-3 bg-secondary/30">
                    ...
                    <button type="button" onClick={() => setOpenTakenYear(null)}><X className="h-4 w-4 text-muted-foreground hover:text-foreground" /></button>
                  </div>
                  <TakenYearBox year={openTakenYear} courses={courses} completedIds={completedSet} onToggle={toggleCompleted} />
                </div>
              );
            })()}
          </div>
              ...
              return (
                <div key={pin.courseId} className="flex items-center gap-1.5 rounded-full border border-border bg-secondary px-2.5 py-1 text-xs">
                  ...
                  <button type="button" onClick={() => handleTogglePin(pin.courseId)}
                    className="ml-0.5 text-muted-foreground hover:text-destructive"><X className="h-3 w-3" /></button>
                </div>
              );
            })}
          </div>
            ...
            {prereqErrors.map((err, i) => (
              <li key={i} className="rounded-xl border border-red-200 bg-white px-4 py-3 dark:border-red-800 dark:bg-red-900/20">
                ...
                <p className="mt-1 text-xs text-red-600 dark:text-red-400">
                  Missing: {err.missing.map((m, j) => <span key={j}><strong>{m.code}</strong> ({m.name}){j < err.missing.length - 1 ? ", " : ""}</span>)}
                </p>
              </li>
            ))}
          </ul>
        ...
        <section ref={planRef} className="space-y-5 scroll-mt-24">
          ...
          <div className="flex gap-6 items-start">
            <aside className="hidden lg:block w-44 shrink-0 sticky top-24 self-start">
              <div className="rounded-2xl border border-border bg-card p-4 shadow-sm"><ColourKey /></div>
            </aside>
            ...
            <div className="flex-1 min-w-0 space-y-4">
              ...
              <p className="text-xs text-muted-foreground">
                Use <strong>← →</strong> arrows beside each year for alternative arrangements.
                Year 4 arrows switch tech elective bundles for your option. Hover any pill to <strong>📌 pin</strong> or <strong>× move</strong> it.
              </p>

              {planByYear.map(([year, slots], idx) => {
                const isElectiveYear = year === 4;

                let variantIdx: number, totalVariants: number, currentVariant: YearVariant;
                let goLeft: () => void, goRight: () => void;
                let dotCount: number, dotActive: number;

                if (isElectiveYear) {
                  variantIdx = safeBundle; totalVariants = bundles.length;
                  dotCount = bundles.length; dotActive = safeBundle;
                  ...
                  totalVariants = variants.length; dotCount = variants.length; dotActive = variantIdx;
                          ...
                          <div className="flex flex-col gap-0.5 mt-1">
                            {Array.from({ length: dotCount }).map((_, i) => (
                              <button key={i} type="button"
                                onClick={isElectiveYear ? () => handleBundleChange(i) : () => setYearVariantIdx((prev) => ({ ...prev, [year]: i }))}
                                className={cn("h-1.5 w-1.5 rounded-full transition-all", i === dotActive ? "bg-foreground" : "bg-border hover:bg-muted-foreground")} />
                            ))}
                          </div>
                        </>
                      )}
                    ...
                    <div className="flex-1 min-w-0">
                      {isElectiveYear && hasAlts && (
                        <div className="mb-1.5 flex items-center gap-2 rounded-xl border border-border bg-secondary/30 px-3 py-2">
                          ...
                          <span className="shrink-0 text-[10px] font-bold text-muted-foreground">{safeBundle + 1}/{bundles.length}</span>
                        </div>
                      )}
                      <YearPlanBox year={year} currentVariant={currentVariant}
              ...
              <div className="rounded-2xl border border-border bg-card px-5 py-4">
                <p className="text-xs font-semibold text-foreground mb-1">Co-op & Exam Policy</p>
                <p className="text-xs text-muted-foreground">
                  MECH co-op involves <strong>3–5 work terms</strong> and adds ~12–20 months.
                  Note the <strong>MECH exam policy</strong>: in courses where exams are ≥50% of the grade,
                  you must achieve a weighted average exam grade of at least 50% to pass — independent of other grades.
                </p>
              </div>
            </div>
          </div>
      ...
      <p className="text-center text-xs text-muted-foreground pt-2">
        Course credits are approximate — MECH 2xx integrated blocks are as published. Always verify with the{" "}
        <a href="https://mech.ubc.ca/" target="_blank" rel="noopener noreferrer"
          className="underline underline-offset-2 hover:text-foreground">UBC Mechanical Engineering department</a>.
      </p>
    </div>
  );
}
