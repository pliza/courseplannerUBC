...
import { ChevronDown, ChevronUp, ChevronLeft, ChevronRight, AlertTriangle, Check, Minus, Plus, GraduationCap, CalendarDays, Pin, X, Sun, Layers, Shuffle } from "lucide-react";
import { getAllIGENCoursesForDisplay, getAllIGENCourses, IGEN_PREREQUISITES, IGEN_ELECTIVE_BUNDLES, IGEN_COMP_STUDIES, type PlanCourse } from "../_data/igen-courses";
...
const CAT_META = {
  required: { label: "Core Required", description: "Mandatory IGEN/MECH/ELEC/CHBE/CIVL courses", pill: "border-slate-300 bg-slate-100 text-slate-800 dark:border-slate-600 dark:bg-slate-800/60 dark:text-slate-200", dot: "bg-slate-400" },
  choice:   { label: "Program Choice", description: "Required — choose one from options listed", pill: "border-amber-300 bg-amber-50 text-amber-900 dark:border-amber-600 dark:bg-amber-900/30 dark:text-amber-200", dot: "bg-amber-400" },
  elective: { label: "Technical Elective", description: "Discipline stream — use ← → to switch streams", pill: "border-blue-300 bg-blue-50 text-blue-900 dark:border-blue-600 dark:bg-blue-900/30 dark:text-blue-200", dot: "bg-blue-400" },
...
function CheckRow({ course, checked, onChange }: { course: PlanCourse; checked: boolean; onChange: (id: string, v: boolean) => void }) {
  return (
    <label className={cn("flex cursor-pointer items-center gap-3 rounded-xl border px-4 py-2.5", checked ? cn("ring-1", CAT_META[course.category].pill, "ring-slate-400") : "border-border bg-card hover:bg-secondary/50")}>
      ...
      {course.note && <span className="shrink-0 rounded-full bg-amber-100 px-2 py-0.5 text-[10px] font-semibold text-amber-700 dark:bg-amber-900/40 dark:text-amber-300 max-w-[180px] truncate">{course.note}</span>}
...
export default function IGENPlanner() {
  ...
  const displayCourses = useMemo(() => getAllIGENCoursesForDisplay(activeBundleIdx), [activeBundleIdx]);
  const allCourses = useMemo(() => getAllIGENCourses(activeBundleIdx, completedSet), [activeBundleIdx, completedSet]);
  const stableCoursesForStanding = useMemo(() => getAllIGENCoursesForDisplay(0), []);
  ...
  const safeBundle = Math.min(activeBundleIdx, IGEN_ELECTIVE_BUNDLES.length - 1);
  const activeBundle = IGEN_ELECTIVE_BUNDLES[safeBundle];

  const rebuildForBundle = useCallback((bi: number, cap?: Record<string, number>) => buildDegreePlan({ allCourses: getAllIGENCourses(bi, completedSet), completedIds: completedSet, startYear: standing, startTerm: 1, defaultCoursesPerTerm: defaultLoad, termCapacities: cap ?? termCapacities, totalDegreeYears: totalYears, pinnedCourses, termExclusions }), [completedSet, standing, defaultLoad, termCapacities, totalYears, pinnedCourses, termExclusions]);
  const handleBundleChange = useCallback((n: number) => { const c = Math.max(0, Math.min(IGEN_ELECTIVE_BUNDLES.length - 1, n)); setActiveBundleIdx(c); if (generated) setPlan(rebuildForBundle(c)); }, [generated, rebuildForBundle]);
  ...
  const handleGenerate = () => {
    const errors = validatePrerequisites(completedSet, displayCourses, IGEN_PREREQUISITES);
  ...
  return (
    <div className="mx-auto max-w-3xl space-y-7 px-4 py-10 sm:px-6">
      <div><p className="mb-1 text-xs font-semibold uppercase tracking-widest text-muted-foreground">B.A.Sc. · Applied Science · IGEN</p><h1 className="text-3xl font-extrabold tracking-tight sm:text-4xl">Degree Planner</h1><p className="mt-1 text-sm text-muted-foreground">Integrated Engineering · ~155 credit program</p></div>
      ...
      <div className="rounded-2xl border border-border bg-card p-6 shadow-sm space-y-5">
        ...
        <div className="flex flex-wrap gap-x-4 gap-y-1 border-t border-border pt-3 text-xs text-muted-foreground">
          ...
          <span className="flex items-center gap-1.5"><Shuffle className="h-3.5 w-3.5" />18cr tech electives — switch discipline stream with ← →</span>
        </div>
      </div>

      <div className="flex items-start gap-3 rounded-xl border border-purple-200 bg-purple-50 px-4 py-3 dark:border-purple-800 dark:bg-purple-950/30">
        <Shuffle className="mt-0.5 h-4 w-4 shrink-0 text-purple-600 dark:text-purple-400" />
        <p className="text-xs text-purple-800 dark:text-purple-200">
          <strong>IGEN Tech Elective Rule:</strong> Of your 18 credits of technical electives (6cr in Y3 + 12cr in Y4), <strong>9 credits must be in one engineering discipline</strong> and <strong>6 credits in a second discipline</strong>. Use the ← → arrows to navigate between discipline streams. Bundles are pre-configured to satisfy this depth/breadth rule. The <strong>three full-year IGEN design projects</strong> (IGEN 230/330/430, 6cr each) are the heart of the program.
        </p>
      </div>
        ...
        <section ref={planRef} className="space-y-5 scroll-mt-24">
          ...
          <div className="flex gap-6 items-start">
            ...
            <div className="flex-1 min-w-0 space-y-4">
              <p className="text-xs text-muted-foreground">Use <strong>← →</strong> for arrangements. Year 4 arrows switch your <strong>discipline stream</strong> (18cr total across Y3+Y4). Y3 tech electives update automatically when you change stream.</p>
              {planByYear.map(([year, slots], idx) => {
                ...
                if (isElec) {
                  variantIdx = safeBundle; totalVariants = IGEN_ELECTIVE_BUNDLES.length; dotCount = IGEN_ELECTIVE_BUNDLES.length; dotActive = safeBundle;
              ...
              <CompCard />
              <div className="rounded-2xl border border-border bg-card px-5 py-4"><p className="text-xs font-semibold mb-1">Co-op (5-year path)</p><p className="text-xs text-muted-foreground">IGEN offers a <strong>5-year co-op path</strong> with eight academic semesters and five co-op work terms. Consult the IGEN Student Advisor to plan your elective streams and co-op scheduling.</p></div>
            </div>
          </div>
        </section>
      )}
      <p className="text-center text-xs text-muted-foreground pt-2">Course names inferred where not in source requirements. Always verify with the <a href="https://apsc.ubc.ca/programs/undergraduate-programs/integrated-engineering" target="_blank" rel="noopener noreferrer" className="underline hover:text-foreground">IGEN Student Advisor</a>.</p>
    </div>
  );
}
