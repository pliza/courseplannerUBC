...
import { ChevronDown, ChevronUp, ChevronLeft, ChevronRight, AlertTriangle, Check, Minus, Plus, GraduationCap, CalendarDays, Pin, X, Sun, Layers, Droplets } from "lucide-react";
import { getAllENVLCoursesForDisplay, getAllENVLCourses, ENVL_PREREQUISITES, ENVL_ELECTIVE_BUNDLES, ENVL_COMP_STUDIES, type PlanCourse } from "../_data/envl-courses";
...
const CAT_META = {
  required: { label: "Core Required", description: "Mandatory CHBE/CIVL/ENVE/EOSC courses", pill: "border-slate-300 bg-slate-100 text-slate-800 dark:border-slate-600 dark:bg-slate-800/60 dark:text-slate-200", dot: "bg-slate-400" },
...
export default function ENVLPlanner() {
  ...
  const displayCourses = useMemo(() => getAllENVLCoursesForDisplay(activeBundleIdx), [activeBundleIdx]);
  const allCourses = useMemo(() => getAllENVLCourses(activeBundleIdx, completedSet), [activeBundleIdx, completedSet]);
  const stableCoursesForStanding = useMemo(() => getAllENVLCoursesForDisplay(0), []);
  ...
  const safeBundle = Math.min(activeBundleIdx, ENVL_ELECTIVE_BUNDLES.length - 1);
  const activeBundle = ENVL_ELECTIVE_BUNDLES[safeBundle];

  const rebuildForBundle = useCallback((bi: number, cap?: Record<string, number>) => buildDegreePlan({ allCourses: getAllENVLCourses(bi, completedSet), completedIds: completedSet, startYear: standing, startTerm: 1, defaultCoursesPerTerm: defaultLoad, termCapacities: cap ?? termCapacities, totalDegreeYears: totalYears, pinnedCourses, termExclusions }), [completedSet, standing, defaultLoad, termCapacities, totalYears, pinnedCourses, termExclusions]);
  const handleBundleChange = useCallback((n: number) => { const c = Math.max(0, Math.min(ENVL_ELECTIVE_BUNDLES.length - 1, n)); setActiveBundleIdx(c); if (generated) setPlan(rebuildForBundle(c)); }, [generated, rebuildForBundle]);
  ...
  const handleGenerate = () => {
    const errors = validatePrerequisites(completedSet, displayCourses, ENVL_PREREQUISITES);
  ...
  return (
    <div className="mx-auto max-w-3xl space-y-7 px-4 py-10 sm:px-6">
      <div><p className="mb-1 text-xs font-semibold uppercase tracking-widest text-muted-foreground">B.A.Sc. · Applied Science · ENVL</p><h1 className="text-3xl font-extrabold tracking-tight sm:text-4xl">Degree Planner</h1><p className="mt-1 text-sm text-muted-foreground">Environmental Engineering · ~153 credit program</p></div>
      ...
      <div className="rounded-2xl border border-border bg-card p-6 shadow-sm space-y-5">
        ...
        <div className="flex flex-wrap gap-x-4 gap-y-1 border-t border-border pt-3 text-xs text-muted-foreground">
          ...
          <span className="flex items-center gap-1.5"><Droplets className="h-3.5 w-3.5" />Multi-disciplinary: CHBE + CIVL + ENVE + EOSC</span>
        </div>
      </div>
      ...
      <div className="flex items-start gap-3 rounded-xl border border-blue-200 bg-blue-50 px-4 py-3 dark:border-blue-800 dark:bg-blue-950/30">
        <Droplets className="mt-0.5 h-4 w-4 shrink-0 text-blue-600 dark:text-blue-400" />
        <p className="text-xs text-blue-800 dark:text-blue-200"><strong>Environmental Engineering</strong> is uniquely multi-disciplinary — courses span Chemical &amp; Biological Engineering (CHBE), Civil Engineering (CIVL), Environmental Engineering (ENVE), and Earth &amp; Ocean Sciences (EOSC). <strong>ENVE 401 is an 8-credit year-long capstone</strong> representing significant multi-disciplinary design work in Year 4.</p>
      </div>
        ...
        <section ref={planRef} className="space-y-5 scroll-mt-24">
          ...
          <div className="flex gap-6 items-start">
            ...
            <div className="flex-1 min-w-0 space-y-4">
              <p className="text-xs text-muted-foreground">Use <strong>← →</strong> for alternative arrangements. Year 4 arrows switch tech elective bundles (6 credits).</p>
              {planByYear.map(([year, slots], idx) => {
                ...
                if (isElec) {
                  variantIdx = safeBundle; totalVariants = ENVL_ELECTIVE_BUNDLES.length; dotCount = ENVL_ELECTIVE_BUNDLES.length; dotActive = safeBundle;
              ...
              <CompCard />
              <div className="rounded-2xl border border-border bg-card px-5 py-4"><p className="text-xs font-semibold mb-1">Co-op Program</p><p className="text-xs text-muted-foreground">ENVL offers optional co-op with up to <strong>five 4-month placements</strong>. Co-op is managed through the Applied Science Co-op Program with winter or summer terms possible.</p></div>
            </div>
          </div>
        </section>
      )}
      <p className="text-center text-xs text-muted-foreground pt-2">Course names inferred where not in source requirements. Always verify with the <a href="https://apsc.ubc.ca/" target="_blank" rel="noopener noreferrer" className="underline hover:text-foreground">UBC Environmental Engineering department</a>.</p>
    </div>
  );
}
