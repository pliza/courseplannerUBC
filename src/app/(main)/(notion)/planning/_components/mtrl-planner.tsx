...
import {
  ...
  CalendarDays, Pin, X, Sun, Layers, Atom,
} from "lucide-react";
import {
  getAllMTRLCoursesForDisplay, getAllMTRLCourses,
  MTRL_PREREQUISITES, MTRL_ELECTIVE_BUNDLES, MTRL_COMP_STUDIES_CATEGORIES,
  type PlanCourse,
} from "../_data/mtrl-courses";
import {
      ...
      <button type="button" onClick={() => setOpen((o) => !o)}
        ...
        <div className="hidden sm:flex items-center gap-2 shrink-0">
          <div className="h-1.5 w-24 overflow-hidden rounded-full bg-secondary">
            <div className="h-full rounded-full bg-foreground transition-all" style={{ width: `${Math.min(100, (totalCr / 40) * 100)}%` }} />
          </div>
      ...
      {open && (
        <div className="border-t border-border px-5 pb-5 pt-4 space-y-4">
          <p className="text-xs text-muted-foreground">
            WRDS 150, MTRL 280, MTRL 381, and APSC 450 together satisfy most Complementary Studies requirements.
            The two Y2 CS elective slots fill the remaining humanities/social sciences credits.
          </p>
          <div className="flex flex-wrap gap-1.5">
            {["WRDS 150 ✓", "MTRL 280 ✓", "MTRL 381 ✓", "APSC 450 ✓", "Y2 CS Elective x2 ✓"].map((t) => (
          ...
          <div className="space-y-1.5">
            {MTRL_COMP_STUDIES_CATEGORIES.map((cat, i) => (
...
export default function MTRLPlanner() {
  ...
  const displayCourses = useMemo(
    () => getAllMTRLCoursesForDisplay(activeBundleIdx),
    [activeBundleIdx]
  );

  const allCourses = useMemo(
    () => getAllMTRLCourses(activeBundleIdx, completedSet),
    [activeBundleIdx, completedSet]
  );

  /**
   * Stable reference for auto-fill — uses bundle 0 always so that
   * Y4 bundle arrow clicks never trigger a plan reset.
   */
  const stableCoursesForStanding = useMemo(
    () => getAllMTRLCoursesForDisplay(0),
    []
  );
  ...
  const safeBundle = Math.min(activeBundleIdx, MTRL_ELECTIVE_BUNDLES.length - 1);
  const activeBundle = MTRL_ELECTIVE_BUNDLES[safeBundle];

  const rebuildForBundle = useCallback((bundleIdx: number, capOverride?: Record<string, number>) => {
    return buildDegreePlan({
      allCourses: getAllMTRLCourses(bundleIdx, completedSet),
  ...
  const handleBundleChange = useCallback((newIdx: number) => {
    const clamped = Math.max(0, Math.min(MTRL_ELECTIVE_BUNDLES.length - 1, newIdx));
  ...
  const handleGenerate = () => {
    const errors = validatePrerequisites(completedSet, displayCourses, MTRL_PREREQUISITES);
  ...
  return (
    <div className="mx-auto max-w-3xl space-y-7 px-4 py-10 sm:px-6">
      ...
      <div>
        <p className="mb-1 text-xs font-semibold uppercase tracking-widest text-muted-foreground">B.A.Sc. · Applied Science · MTRL</p>
        <h1 className="text-3xl font-extrabold tracking-tight text-foreground sm:text-4xl">Degree Planner</h1>
        <p className="mt-1 text-sm text-muted-foreground">
          Materials Engineering · ~156 credit program
        </p>
      </div>
      ...
      <div className="rounded-2xl border border-border bg-card p-6 shadow-sm space-y-5">
        ...
        <div className="flex flex-wrap gap-x-4 gap-y-1 border-t border-border pt-3">
          ...
          <p className="text-xs text-muted-foreground flex items-center gap-1.5">
            <Atom className="h-3.5 w-3.5" />Materials Engineering — no formal specialization tracks
          </p>
        </div>
      </div>
      ...
      <div className="flex items-start gap-3 rounded-xl border border-amber-300 bg-amber-50 px-4 py-3 dark:border-amber-700 dark:bg-amber-950/30">
        ...
        <p className="text-xs text-amber-800 dark:text-amber-200">
          <strong>Year 2 is foundational and credit-heavy (39 credits).</strong> MTRL 250 (Thermodynamics) and
          MTRL 264 (Structure) are prerequisites for most third-year courses — prioritise these early.
          Consider taking MATH 253 and MATH 255 alongside MTRL 250/251 rather than sequentially.
        </p>
      </div>
          ...
          <div className="flex gap-6 items-start">
            ...
            <div className="flex-1 min-w-0 space-y-4">
              ...
              <p className="text-xs text-muted-foreground">
                ...
                Year 4 arrows switch the tech elective bundle (21 credits). Hover any pill to <strong>📌 pin</strong> or <strong>× move</strong> it.
              </p>
              ...
              {planByYear.map(([year, slots], idx) => {
                ...
                if (isElectiveYear) {
                  variantIdx = safeBundle; totalVariants = MTRL_ELECTIVE_BUNDLES.length;
                  dotCount = MTRL_ELECTIVE_BUNDLES.length; dotActive = safeBundle;
                    ...
                    <div className="flex-1 min-w-0">
                      {isElectiveYear && hasAlts && (
                        <div className="mb-1.5 flex items-center gap-2 rounded-xl border border-border bg-secondary/30 px-3 py-2">
                          ...
                          <span className="shrink-0 text-[10px] font-bold text-muted-foreground">{safeBundle + 1}/{MTRL_ELECTIVE_BUNDLES.length}</span>
                        </div>
                      )}
              ...
              <div className="rounded-2xl border border-border bg-card px-5 py-4">
                ...
                <p className="text-xs text-muted-foreground">
                  MTRL offers an optional <strong>Co-operative Education</strong> program providing up to{" "}
                  <strong>20 months of industry experience</strong> over the final three years of the program.
                  Co-op adds work terms between academic sessions; consult the MTRL Co-op coordinator for scheduling implications.
                </p>
              </div>
            </div>
          </div>
      ...
      <p className="text-center text-xs text-muted-foreground pt-2">
        Year 3 course names and Year 4 elective codes are inferred where not explicitly listed in the source requirements.
        Always verify with the{" "}
        <a href="https://mtrl.ubc.ca/" target="_blank" rel="noopener noreferrer"
          className="underline underline-offset-2 hover:text-foreground">UBC Materials Engineering department</a>.
      </p>
    </div>
  );
}
