...
import { ChevronDown, ChevronUp, ChevronLeft, ChevronRight, AlertTriangle, Check, Minus, Plus, GraduationCap, CalendarDays, Pin, X, Sun, Layers, Wrench } from "lucide-react";
import { getAllMANUCoursesForDisplay, getAllMANUCourses, MANU_PREREQUISITES, MANU_ELECTIVE_BUNDLES, MANU_COMP_STUDIES, type PlanCourse } from "../_data/manu-courses";
...
const CAT_META = {
  required: { label: "Core Required", description: "Mandatory MANU/MECH/MTRL courses", pill: "border-slate-300 bg-slate-100 text-slate-800 dark:border-slate-600 dark:bg-slate-800/60 dark:text-slate-200", dot: "bg-slate-400" },
  choice:   { label: "Program Choice", description: "Required with options", pill: "border-amber-300 bg-amber-50 text-amber-900 dark:border-amber-600 dark:bg-amber-900/30 dark:text-amber-200", dot: "bg-amber-400" },
...
function Sel({ value, onChange, children }: { value: string; onChange: (v: string) => void; children: React.ReactNode }) {
  return <select value={value} onChange={(e) => onChange(e.target.value)} className="w-full rounded-xl border border-border bg-background px-3 py-2 text-sm text-foreground focus:outline-none focus:ring-1 focus:ring-foreground dark:bg-card">{children}</select>;
}
...
function Pill({ course, pinned, onPin, onEject }: { course: PlanCourse; pinned?: boolean; onPin?: () => void; onEject?: () => void }) {
  return (
    <span title={`${course.name}${course.note ? ` — ${course.note}` : ""}`} className={cn("group inline-flex items-center gap-1.5 rounded-lg border px-2 py-1 text-xs font-medium transition-colors", CAT_META[course.category].pill, pinned && "ring-1 ring-offset-1 ring-foreground/40")}>
      ...
      {onPin && <button type="button" onClick={(e) => { e.stopPropagation(); onPin(); }} className="opacity-0 group-hover:opacity-100 transition-opacity" title={pinned ? "Remove pin" : "Pin"}><Pin className={cn("h-2.5 w-2.5", pinned ? "fill-current" : "")} /></button>}
      {onEject && <button type="button" onClick={(e) => { e.stopPropagation(); onEject(); }} className="opacity-0 group-hover:opacity-100 transition-opacity text-muted-foreground hover:text-destructive" title="Move to another term"><X className="h-2.5 w-2.5" /></button>}
    </span>
  );
}
function CheckRow({ course, checked, onChange }: { course: PlanCourse; checked: boolean; onChange: (id: string, v: boolean) => void }) {
  return (
    <label className={cn("flex cursor-pointer items-center gap-3 rounded-xl border px-4 py-2.5 transition-colors", checked ? cn("ring-1", CAT_META[course.category].pill, "ring-slate-400") : "border-border bg-card hover:bg-secondary/50")}>
      ...
      {course.note && <span className="shrink-0 rounded-full bg-amber-100 px-2 py-0.5 text-[10px] font-semibold text-amber-700 dark:bg-amber-900/40 dark:text-amber-300">{course.note}</span>}
...
function ColourKey() {
  return (
    <div className="space-y-3">
      ...
      {(Object.entries(CAT_META) as [keyof typeof CAT_META, (typeof CAT_META)[keyof typeof CAT_META]][]).map(([, m]) => (
        <div key={m.label} className="flex items-start gap-2.5"><span className={cn("mt-0.5 h-3 w-3 shrink-0 rounded-full", m.dot)} /><div><p className="text-xs font-semibold text-foreground">{m.label}</p><p className="text-[11px] text-muted-foreground leading-snug">{m.description}</p></div></div>
      ))}
    </div>
  );
}
  ...
  return (
    <div className={cn("px-5 py-4", isSummer && "bg-amber-50/40 dark:bg-amber-950/10")}>
      <div className="mb-3 flex items-center justify-between gap-3">
        <div><p className={cn("text-xs font-bold uppercase tracking-widest", isSummer ? "text-amber-700 dark:text-amber-400" : "text-muted-foreground")}>{slot.displayLabel}{!isSummer && (slot.term === 1 ? " · Sep–Dec" : " · Jan–Apr")}</p><p className="text-[11px] text-muted-foreground">{slot.courses.length} courses · {slot.totalCredits} cr{slot.overCapacity && <span className="ml-1 text-amber-600"> ⚠ over cap</span>}</p></div>
        {!isSummer && <div className="flex items-center gap-1 rounded-full border border-border px-1.5 py-0.5 shrink-0"><button type="button" onClick={() => onCapacityChange(slot.label, Math.max(1, slot.capacity - 1))} className="flex h-5 w-5 items-center justify-center rounded-full text-muted-foreground hover:bg-secondary"><Minus className="h-2.5 w-2.5" /></button><span className="w-5 text-center text-[11px] font-bold tabular-nums">{slot.capacity}</span><button type="button" onClick={() => onCapacityChange(slot.label, Math.min(9, slot.capacity + 1))} className="flex h-5 w-5 items-center justify-center rounded-full text-muted-foreground hover:bg-secondary"><Plus className="h-2.5 w-2.5" /></button><span className="ml-1 text-[10px] text-muted-foreground">max</span></div>}
      </div>
      ...
      {!isSummer && <div className="mt-3">{showPin ? (<div className="flex items-center gap-2"><select className="flex-1 rounded-lg border border-border bg-background px-2 py-1.5 text-xs focus:outline-none dark:bg-card" defaultValue="" onChange={(e) => { if (e.target.value) { onAddPin(e.target.value, slot.academicYear, slot.term); setShowPin(false); } }}><option value="" disabled>Select a course to pin…</option>{availableToPin.map((c) => <option key={c.id} value={c.id}>{c.code} — {c.name}</option>)}</select><button type="button" onClick={() => setShowPin(false)} className="rounded-lg border border-border p-1.5"><X className="h-3.5 w-3.5" /></button></div>) : (<button type="button" onClick={() => setShowPin(true)} className="flex items-center gap-1.5 text-[11px] text-muted-foreground hover:text-foreground"><Pin className="h-3 w-3" /> Pin a course to this term</button>)}</div>}
    </div>
  );
}
function YearBox({ year, currentVariant, variantIdx, totalVariants, pinnedIds, onTogglePin, onCapacityChange, onAddPin, onEject, availableToPin, defaultOpen }: { year: number; currentVariant: YearVariant; variantIdx: number; totalVariants: number; pinnedIds: ReadonlySet<string>; onTogglePin: (id: string) => void; onCapacityChange: (label: string, cap: number) => void; onAddPin: (id: string, year: number, term: 1 | 2) => void; onEject: (id: string, term: 1 | 2) => void; availableToPin: PlanCourse[]; defaultOpen: boolean }) {
  ...
  return (
    <div className="w-full overflow-hidden rounded-2xl border border-border bg-card shadow-sm">
      <button type="button" onClick={() => setOpen((o) => !o)} className="flex w-full items-center gap-4 px-5 py-4 text-left hover:bg-secondary/40">
        <span className="flex h-9 w-9 shrink-0 items-center justify-center rounded-full bg-foreground text-sm font-extrabold text-background">{year}</span>
        <div className="flex-1 min-w-0"><div className="flex items-center gap-2"><p className="font-bold text-foreground">Year {year}</p>{totalVariants > 1 && <span className="rounded-full bg-blue-100 px-2 py-0.5 text-[10px] font-semibold text-blue-700 dark:bg-blue-900/30 dark:text-blue-300">{variantIdx + 1}/{totalVariants}</span>}</div><p className="text-xs text-muted-foreground">{currentVariant.label} · {totalC} courses · {totalCr} credits</p></div>
      ...
      {open && <div className="border-t border-border divide-y divide-border">{currentVariant.slots.map((sl) => <TermSec key={sl.label} slot={sl} pinnedIds={pinnedIds} onTogglePin={onTogglePin} onCapacityChange={onCapacityChange} onAddPin={onAddPin} onEject={onEject} availableToPin={availableToPin} />)}</div>}
    </div>
  );
}
...
function CompCard() {
  ...
  return (
    <div className="rounded-2xl border border-border bg-card shadow-sm overflow-hidden">
      <button type="button" onClick={() => setOpen((o) => !o)} className="flex w-full items-center gap-4 px-5 py-4 text-left hover:bg-secondary/40">
        <span className="flex h-9 w-9 shrink-0 items-center justify-center rounded-full bg-emerald-100 dark:bg-emerald-900/40"><GraduationCap className="h-4 w-4 text-emerald-700 dark:text-emerald-400" /></span>
        <div className="flex-1"><p className="font-bold text-foreground">Complementary Studies</p><p className="text-xs text-muted-foreground">Min 20 credits · Click to expand</p></div>
        {open ? <ChevronUp className="h-5 w-5 shrink-0 text-muted-foreground" /> : <ChevronDown className="h-5 w-5 shrink-0 text-muted-foreground" />}
      </button>
      {open && (<div className="border-t border-border px-5 pb-5 pt-4 space-y-3"><p className="text-xs text-muted-foreground">WRDS 150, MECH 400, MECH 491, and the two CS elective slots satisfy most CS requirements.</p><div className="space-y-1.5">{MANU_COMP_STUDIES.map((cat, i) => (<div key={i} className="rounded-xl border border-border px-4 py-2.5"><div className="flex items-center justify-between"><span className="text-xs font-semibold">{cat.label}</span><span className="text-[10px] text-muted-foreground">min {cat.minCredits} cr</span></div><div className="flex flex-wrap gap-1 mt-1">{cat.examples.map((e) => <span key={e} className="rounded-md border border-border bg-card px-2 py-0.5 text-[11px]">{e}</span>)}</div></div>))}</div></div>)}
    </div>
  );
}

export default function MANUPlanner() {
  ...
  const displayCourses = useMemo(() => getAllMANUCoursesForDisplay(activeBundleIdx), [activeBundleIdx]);
  const allCourses = useMemo(() => getAllMANUCourses(activeBundleIdx, completedSet), [activeBundleIdx, completedSet]);
  const stableCoursesForStanding = useMemo(() => getAllMANUCoursesForDisplay(0), []);
  ...
  const safeBundle = Math.min(activeBundleIdx, MANU_ELECTIVE_BUNDLES.length - 1);
  const activeBundle = MANU_ELECTIVE_BUNDLES[safeBundle];

  const rebuildForBundle = useCallback((bi: number, capOverride?: Record<string, number>) => buildDegreePlan({ allCourses: getAllMANUCourses(bi, completedSet), completedIds: completedSet, startYear: standing, startTerm: 1, defaultCoursesPerTerm: defaultLoad, termCapacities: capOverride ?? termCapacities, totalDegreeYears: totalYears, pinnedCourses, termExclusions }), [completedSet, standing, defaultLoad, termCapacities, totalYears, pinnedCourses, termExclusions]);
  const handleBundleChange = useCallback((n: number) => { const c = Math.max(0, Math.min(MANU_ELECTIVE_BUNDLES.length - 1, n)); setActiveBundleIdx(c); if (generated) setPlan(rebuildForBundle(c)); }, [generated, rebuildForBundle]);
  ...
  const handleGenerate = () => {
    const errors = validatePrerequisites(completedSet, displayCourses, MANU_PREREQUISITES);
  ...
  return (
    <div className="mx-auto max-w-3xl space-y-7 px-4 py-10 sm:px-6">
      <div><p className="mb-1 text-xs font-semibold uppercase tracking-widest text-muted-foreground">B.A.Sc. · Applied Science · MANU</p><h1 className="text-3xl font-extrabold tracking-tight text-foreground sm:text-4xl">Degree Planner</h1><p className="mt-1 text-sm text-muted-foreground">Manufacturing Engineering (Vancouver &amp; Okanagan) · ~154 credit program</p></div>
      ...
      <div className="rounded-2xl border border-border bg-card p-6 shadow-sm space-y-5">
        ...
        <div className="flex flex-wrap gap-x-4 gap-y-1 border-t border-border pt-3 text-xs text-muted-foreground">
          ...
          <span className="flex items-center gap-1.5"><Wrench className="h-3.5 w-3.5" />Joint MECH/MTRL program · significant CPEN content in Y2–Y3</span>
        </div>
      </div>
      ...
      <div className="flex items-start gap-3 rounded-xl border border-amber-300 bg-amber-50 px-4 py-3 dark:border-amber-700 dark:bg-amber-950/30">
        ...
        <p className="text-xs text-amber-800 dark:text-amber-200"><strong>MANU is an interdisciplinary program</strong> combining courses from Mechanical, Materials, and Computer Engineering. MANU 330 and MANU 430 are <strong>6-credit year-long project courses</strong> — they appear in both terms and represent substantial project work. Year 2 is credit-intensive at 38 credits.</p>
      </div>
      ...
      <div className="rounded-2xl border border-border bg-card overflow-hidden shadow-sm">
        ...
        {showTaken && (
          <div className="border-t border-border px-5 pb-5 pt-4">
            <div className="grid grid-cols-4 gap-3 mb-4">{displayByYear.map(([year, courses]) => { const done = courses.filter((c) => completedSet.has(c.id)).length; const isOpen = openTakenYear === year; return (<button key={year} type="button" onClick={() => setOpenTakenYear(isOpen ? null : year)} className={cn("rounded-2xl border px-3 py-3 text-left transition-all", isOpen ? "border-foreground bg-secondary/40 ring-1 ring-foreground" : "border-border bg-card hover:border-foreground/40")}><p className="text-2xl font-extrabold">{year}</p><p className="text-[11px] font-medium mt-1">Year {year}</p><p className="text-[11px] text-muted-foreground">{done}/{courses.length}</p>{done === courses.length && courses.length > 0 && <p className="text-[10px] font-bold text-emerald-600">✓ All done</p>}</button>); })}</div>
            {openTakenYear !== null && (() => { const courses = displayByYear.find(([y]) => y === openTakenYear)?.[1] ?? []; return (<div className="rounded-2xl border border-foreground overflow-hidden"><div className="flex items-center justify-between px-5 py-3 bg-secondary/30"><p className="text-sm font-bold">Year {openTakenYear} Courses</p><button type="button" onClick={() => setOpenTakenYear(null)}><X className="h-4 w-4 text-muted-foreground" /></button></div><TakenBox year={openTakenYear} courses={courses} completedIds={completedSet} onToggle={toggleCompleted} /></div>); })()}
          </div>
        )}
      </div>

      {pinnedCourses.length > 0 && <div className="rounded-xl border border-border bg-card px-4 py-3"><div className="flex items-center gap-2 mb-2"><Pin className="h-3.5 w-3.5 text-muted-foreground" /><p className="text-xs font-semibold">Pinned ({pinnedCourses.length})</p></div><div className="flex flex-wrap gap-2">{pinnedCourses.map((pin) => { const c = allCourses.find((x) => x.id === pin.courseId); if (!c) return null; return (<div key={pin.courseId} className="flex items-center gap-1.5 rounded-full border border-border bg-secondary px-2.5 py-1 text-xs"><span className="font-mono font-bold">{c.code}</span><span className="text-muted-foreground">→ Y{pin.year}·T{pin.term}</span><button type="button" onClick={() => handleTogglePin(pin.courseId)}><X className="h-3 w-3 text-muted-foreground hover:text-destructive" /></button></div>); })}</div></div>}

      {!feasibility.ok && <div className="flex items-start gap-3 rounded-xl border border-amber-300 bg-amber-50 px-4 py-3 dark:border-amber-700 dark:bg-amber-950/30"><AlertTriangle className="mt-0.5 h-4 w-4 shrink-0 text-amber-600" /><p className="text-xs text-amber-800 dark:text-amber-200">{feasibility.message}</p></div>}
      ...
      {prereqErrors.length > 0 && <div className="rounded-2xl border border-red-300 bg-red-50 p-5 dark:border-red-700 dark:bg-red-950/30"><div className="mb-3 flex items-center gap-2"><AlertTriangle className="h-4 w-4 text-red-600" /><p className="text-sm font-bold text-red-700 dark:text-red-300">Prerequisite Issues</p></div><ul className="space-y-3">{prereqErrors.map((err, i) => (<li key={i} className="rounded-xl border border-red-200 bg-white px-4 py-3 dark:border-red-800 dark:bg-red-900/20"><p className="text-sm font-bold text-red-700 dark:text-red-300">{err.courseCode} — {err.courseName}</p><p className="mt-1 text-xs text-red-600 dark:text-red-400">Missing: {err.missing.map((m, j) => <span key={j}><strong>{m.code}</strong>{j < err.missing.length - 1 ? ", " : ""}</span>)}</p></li>))}</ul></div>}
        ...
        <section ref={planRef} className="space-y-5 scroll-mt-24">
          ...
          <div className="flex gap-6 items-start">
            ...
            <div className="flex-1 min-w-0 space-y-4">
              <p className="text-xs text-muted-foreground">Use <strong>← →</strong> arrows for alternative term arrangements. Year 4 arrows switch tech elective bundles (9 credits).</p>
              {planByYear.map(([year, slots], idx) => {
                ...
                if (isElec) {
                  variantIdx = safeBundle; totalVariants = MANU_ELECTIVE_BUNDLES.length; dotCount = MANU_ELECTIVE_BUNDLES.length; dotActive = safeBundle;
                ...
                return (
                  <div key={year} className="flex items-start gap-2">
                    <div className="flex shrink-0 flex-col items-center justify-start pt-3.5 gap-1" style={{ width: 32 }}>
                      {hasAlts && (<><button type="button" disabled={variantIdx === 0} onClick={goLeft} className="flex h-8 w-8 items-center justify-center rounded-full border border-border bg-card text-muted-foreground shadow-sm hover:bg-secondary disabled:opacity-25"><ChevronLeft className="h-4 w-4" /></button><div className="flex flex-col gap-0.5 mt-1">{Array.from({ length: dotCount }).map((_, i) => <button key={i} type="button" onClick={isElec ? () => handleBundleChange(i) : () => setYearVariantIdx((prev) => ({ ...prev, [year]: i }))} className={cn("h-1.5 w-1.5 rounded-full", i === dotActive ? "bg-foreground" : "bg-border hover:bg-muted-foreground")} />)}</div></>)}
                    </div>
                    <div className="flex-1 min-w-0">
                      {isElec && hasAlts && <div className="mb-1.5 flex items-center gap-2 rounded-xl border border-border bg-secondary/30 px-3 py-2"><Layers className="h-3 w-3 text-muted-foreground" /><p className="text-[11px] text-muted-foreground flex-1 truncate"><span className="font-semibold text-foreground">{activeBundle.label}</span> — {activeBundle.description}</p><span className="text-[10px] font-bold text-muted-foreground">{safeBundle + 1}/{MANU_ELECTIVE_BUNDLES.length}</span></div>}
                    ...
                    <div className="flex shrink-0 flex-col items-center justify-start pt-3.5" style={{ width: 32 }}>{hasAlts && <button type="button" disabled={variantIdx >= totalVariants - 1} onClick={goRight} className="flex h-8 w-8 items-center justify-center rounded-full border border-border bg-card text-muted-foreground shadow-sm hover:bg-secondary disabled:opacity-25"><ChevronRight className="h-4 w-4" /></button>}</div>
                  </div>
                );
              })}
              <CompCard />
              <div className="rounded-2xl border border-border bg-card px-5 py-4"><p className="text-xs font-semibold mb-1">Co-op Program</p><p className="text-xs text-muted-foreground">Manufacturing Engineering offers optional co-op. Consult the MANU co-op coordinator for term scheduling. The program is jointly administered by the Departments of Mechanical and Materials Engineering.</p></div>
            </div>
          </div>
        </section>
      )}

      <p className="text-center text-xs text-muted-foreground pt-2">Course names inferred where not in source requirements. Always verify with the <a href="https://manu.ubc.ca/" target="_blank" rel="noopener noreferrer" className="underline hover:text-foreground">UBC MANU department</a>.</p>
    </div>
  );
}
