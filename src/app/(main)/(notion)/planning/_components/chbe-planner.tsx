...
import {
  ...
  CalendarDays, Pin, X, Sun, FlaskConical, Layers,
} from "lucide-react";
import {
  getAllCHBECoursesForDisplay, getAllCHBECourses,
  CHBE_PREREQUISITES, CHBE_ELECTIVE_BUNDLES,
  type PlanCourse, type CHTrack,
} from "../_data/chbe-courses";
import {
...
// ─── Category colours (same as BME planner) ───────────────────────────────

const CAT_META = {
  required: {
    label: "Core Required", description: "Must-take CHBE/CH program courses",
    pill: "border-slate-300 bg-slate-100 text-slate-800 dark:border-slate-600 dark:bg-slate-800/60 dark:text-slate-200",
    dot: "bg-slate-400",
  },
  choice: {
    label: "Track-Specific", description: "Different for CH vs CHBE streams",
    pill: "border-amber-300 bg-amber-50 text-amber-900 dark:border-amber-600 dark:bg-amber-900/30 dark:text-amber-200",
    dot: "bg-amber-400",
  },
  elective: {
    label: "Technical Elective", description: "Your 4 chosen tech elective courses",
...
function CourseCheckRow({ course, checked, onChange }: {
  ...
  return (
    <label className={cn(
      ...
      checked ? cn("ring-1", CAT_META[course.category].pill,
...
export default function CHBEPlanner() {
  const planRef = useRef<HTMLDivElement>(null);

  // Config
  const [track, setTrack] = useState<CHTrack>("chbe");
  ...
  /** Which elective bundle is currently active (0 = first bundle) */
  ...
  const displayCourses = useMemo(
    () => getAllCHBECoursesForDisplay(track, activeBundleIdx),
    [track, activeBundleIdx]
  );

  const allCourses = useMemo(
    () => getAllCHBECourses(track, activeBundleIdx, completedSet),
    [track, activeBundleIdx, completedSet]
  );

  /**
   * Stable course list used ONLY for the auto-fill useEffect.
   * Intentionally ignores activeBundleIdx so that switching elective bundles
   * (via Year 4 arrows) never triggers a plan reset.
   */
  const stableCoursesForStanding = useMemo(
    () => getAllCHBECoursesForDisplay(track, 0),
    [track]
  );

  // Auto-fill completed courses based on academic standing.
  // Depends only on `standing` and `track` — NOT on the active elective bundle.
  useEffect(() => {
  ...
  // Reset bundle index when track changes
  useEffect(() => { setActiveBundleIdx(0); }, [track]);
  ...
  /**
   * Rebuild plan for a specific elective bundle.
   * Respects all current state: capacities, pins, exclusions.
   */
  const rebuildForBundle = useCallback((bundleIdx: number, capOverride?: Record<string, number>) => {
    const bundleCourses = getAllCHBECourses(track, bundleIdx, completedSet);
  ...
  }, [track, completedSet, standing, defaultLoad, termCapacities, totalYears, pinnedCourses, termExclusions]);
  ...
  const handleGenerate = () => {
    const errors = validatePrerequisites(completedSet, displayCourses, CHBE_PREREQUISITES);
  ...
  const trackLabel = track === "chbe" ? "Chemical and Biological Engineering" : "Chemical Engineering";
    ...
    <div className="mx-auto max-w-3xl space-y-7 px-4 py-10 sm:px-6">
      ...
      <div>
        <p className="mb-1 text-xs font-semibold uppercase tracking-widest text-muted-foreground">
          B.A.Sc. · Applied Science · CHBE
        </p>
        <h1 className="text-3xl font-extrabold tracking-tight text-foreground sm:text-4xl">
          Degree Planner
        </h1>
        <p className="mt-1 text-sm text-muted-foreground">
          {trackLabel} · Entry after First Year · ~129–133 credit program
        </p>
      </div>
      ...
      <div className="rounded-2xl border border-border bg-card p-6 shadow-sm space-y-5">
        ...
        <div className="grid grid-cols-2 gap-4 sm:grid-cols-4">
          <Field label="Program Stream">
            <Select value={track} onChange={(v) => { setTrack(v as CHTrack); setGenerated(false); }}>
              <option value="chbe">Chem & Bio Eng (CHBE)</option>
              <option value="ch">Chemical Eng (CH)</option>
            </Select>
          </Field>
        ...
        <div className="flex flex-wrap gap-x-4 gap-y-1 border-t border-border pt-3">
          ...
          <p className={cn("text-xs font-semibold flex items-center gap-1.5", track === "chbe" ? "text-emerald-600 dark:text-emerald-400" : "text-blue-600 dark:text-blue-400")}>
            <FlaskConical className="h-3.5 w-3.5" />
            {track === "chbe" ? "CHBE stream — bio-focused" : "CH stream — chemistry-focused"}
          </p>
        </div>
      </div>
      ...
      <div className="flex items-start gap-3 rounded-xl border border-amber-300 bg-amber-50 px-4 py-3 dark:border-amber-700 dark:bg-amber-950/30">
        ...
        <p className="text-xs text-amber-800 dark:text-amber-200">
          <strong>WRDS 150 is a prerequisite for CHBE 201.</strong> You must complete it by end of Year 1 (before September of Year 2) — otherwise CHBE 201 cannot be taken in Year 2, which may add a year to your degree.
        </p>
      </div>
          ...
          <div className="flex gap-6 items-start">
            ...
            <div className="flex-1 min-w-0 space-y-4">
              ...
              {planByYear.map(([year, slots], idx) => {
                // Final year: ← → switches elective bundles
                // Other years: ← → switches term arrangements
                const isElectiveYear = year === totalYears;
                ...
                if (isElectiveYear) {
                  ...
                  totalVariants = bundles.length;
                  dotCount = bundles.length;
                  dotActive = safeBundle;
                  // Variant is the current plan's Year-N slots, labelled with bundle name
                ...
                return (
                  <div key={year} className="flex items-start gap-2">
                    ...
                    {/* Year box */}
                    <div className="flex-1 min-w-0">
                      {/* Elective bundle description strip for the final year */}
              ...
              <div className="rounded-2xl border border-border bg-card px-5 py-4">
                ...
                <p className="text-xs text-muted-foreground">
                  CHBE co-op is highly encouraged and typically involves <strong>five 4-month work terms</strong>.
                  Co-op adds roughly 20 months — plan for 5+ years if participating.
                  Work terms are not shown in this planner; check the{" "}
                  <a href="https://chbe.ubc.ca/" target="_blank" rel="noopener noreferrer"
                    className="underline underline-offset-2 hover:text-foreground">CHBE department website</a>.
                </p>
              </div>
            </div>
          </div>
      ...
      <p className="text-center text-xs text-muted-foreground pt-2">
        Course credits are approximate. Always verify requirements with your academic advisor and the{" "}
        <a href="https://chbe.ubc.ca/" target="_blank" rel="noopener noreferrer"
          className="underline underline-offset-2 hover:text-foreground">UBC CHBE department</a>.
      </p>
    </div>
  );
}
