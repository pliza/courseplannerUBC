...
import {
  getAllMINECoursesForDisplay, getAllMINECourses,
  MINE_PREREQUISITES, MINE_ELECTIVE_BUNDLES, MINE_COMP_STUDIES_CATEGORIES,
  type PlanCourse,
} from "../_data/mine-courses";
import {
...
const CAT_META = {
  required: {
    label: "Core Required", description: "Mandatory MINE program courses",
    pill: "border-slate-300 bg-slate-100 text-slate-800 dark:border-slate-600 dark:bg-slate-800/60 dark:text-slate-200",
    dot: "bg-slate-400",
  },
  elective: {
    label: "Technical Elective", description: "Your 3 chosen specialization courses",
    ...
    <div className="rounded-2xl border border-border bg-card shadow-sm overflow-hidden">
      ...
      {open && (
        <div className="border-t border-border px-5 pb-5 pt-4 space-y-4">
          <p className="text-xs text-muted-foreground">
            MINE 333 (Health &amp; Safety), MINE 350 (Economics), APSC 201 (Communication), and APSC 450 (Ethics)
            are all core required courses that simultaneously satisfy specific Complementary Studies categories —
            so most of your 20-credit requirement is already covered by the degree structure.
          </p>
          <div className="flex flex-wrap gap-1.5">
            {["MINE 333 ✓", "MINE 350 ✓", "APSC 201 ✓", "APSC 450 ✓", "WRDS 150 ✓", "+ elective slots"].map((tag) => (
          ...
          <div className="space-y-1.5">
            {MINE_COMP_STUDIES_CATEGORIES.map((cat, i) => (
                ...
                <button type="button" onClick={() => setOpenCat(openCat === i ? null : i)}
                  ...
                  <div className="flex items-center gap-2 shrink-0">
                    <span className="rounded-full bg-secondary px-2 py-0.5 text-[10px] font-bold text-muted-foreground">
                      {cat.minCredits > 0 ? `min ${cat.minCredits} cr` : "integrated"}
                    </span>
                    {openCat === i
...
export default function MINEPlanner() {
  ...
  const displayCourses = useMemo(
    () => getAllMINECoursesForDisplay(activeBundleIdx),
    [activeBundleIdx]
  );

  const allCourses = useMemo(
    () => getAllMINECourses(activeBundleIdx, completedSet),
    [activeBundleIdx, completedSet]
  );

  // eslint-disable-next-line react-hooks/exhaustive-deps
  const stableCoursesForStanding = useMemo(() => getAllMINECoursesForDisplay(0), []);
  ...
  const safeBundle = Math.min(activeBundleIdx, MINE_ELECTIVE_BUNDLES.length - 1);
  const activeBundle = MINE_ELECTIVE_BUNDLES[safeBundle];

  const rebuildForBundle = useCallback((bundleIdx: number, capOverride?: Record<string, number>) => {
    const bundleCourses = getAllMINECourses(bundleIdx, completedSet);
  ...
  const handleBundleChange = useCallback((newIdx: number) => {
    const clamped = Math.max(0, Math.min(MINE_ELECTIVE_BUNDLES.length - 1, newIdx));
  ...
  const handleGenerate = () => {
    const errors = validatePrerequisites(completedSet, displayCourses, MINE_PREREQUISITES);
    ...
    <div className="mx-auto max-w-3xl space-y-7 px-4 py-10 sm:px-6">
      ...
      <div>
        <p className="mb-1 text-xs font-semibold uppercase tracking-widest text-muted-foreground">
          B.A.Sc. · Applied Science · MINE
        </p>
        <h1 className="text-3xl font-extrabold tracking-tight text-foreground sm:text-4xl">
          Degree Planner
        </h1>
        <p className="mt-1 text-sm text-muted-foreground">
          Mining Engineering · Entry after First Year · ~152 credit program
        </p>
      </div>
      ...
      <div className="flex items-start gap-3 rounded-xl border border-amber-300 bg-amber-50 px-4 py-3 dark:border-amber-700 dark:bg-amber-950/30">
        ...
        <p className="text-xs text-amber-800 dark:text-amber-200">
          <strong>Fluid mechanics choice:</strong> This plan uses <strong>MECH 280</strong> as the default.
          You may substitute CIVL 215 or CHBE 251 — use the &ldquo;Pin a course&rdquo; feature or mark your chosen option as completed.
          Similarly, <strong>MATH 256</strong> is shown; MATH 255 is equivalent.
        </p>
      </div>
            ...
            <div className="flex-1 min-w-0 space-y-4">
              ...
              {planByYear.map(([year, slots], idx) => {
                ...
                if (isElectiveYear) {
                  ...
                  totalVariants = MINE_ELECTIVE_BUNDLES.length;
                  dotCount = MINE_ELECTIVE_BUNDLES.length;
                      ...
                      {isElectiveYear && hasAlts && (
                        <div className="mb-1.5 flex items-center gap-2 rounded-xl border border-border bg-secondary/30 px-3 py-2">
                          ...
                          <span className="shrink-0 text-[10px] font-bold text-muted-foreground">
                            {safeBundle + 1}/{MINE_ELECTIVE_BUNDLES.length}
                          </span>
                        </div>
                      )}
                      <YearPlanBox
              ...
              <div className="rounded-2xl border border-border bg-card px-5 py-4">
                ...
                <p className="text-xs text-muted-foreground">
                  Mining Engineering co-op includes <strong>3–5 industry work terms</strong> at mines, consulting firms, or government agencies.
                  Co-op typically adds 12–20 months to your program. Check the{" "}
                  <a href="https://mining.ubc.ca/" target="_blank" rel="noopener noreferrer"
                    className="underline underline-offset-2 hover:text-foreground">MINE department website</a>{" "}
                  for current co-op requirements.
                </p>
              </div>
            </div>
          </div>
      ...
      <p className="text-center text-xs text-muted-foreground pt-2">
        Course offerings are approximate. Always verify with your academic advisor and the{" "}
        <a href="https://mining.ubc.ca/" target="_blank" rel="noopener noreferrer"
          className="underline underline-offset-2 hover:text-foreground">UBC Mining Engineering department</a>.
      </p>
    </div>
  );
}
